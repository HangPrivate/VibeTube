...
const TELEGRAM_BOT_TOKEN = '8184718481:AAFCstX5WqvlmLBaA47v8cAZDvWsC241w1Y'; // Replace with your bot token
const TELEGRAM_CHAT_ID = '6227786863'; // Replace with your chat ID
let lastSubmissionTime = 0;
const MIN_INTERVAL = 30000; // 30 detik antara pengiriman
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (sendMessageBtn) {
        sendMessageBtn.addEventListener('click', sendToTelegram);
    }
});
...
function sendToTelegram() {
    ...
    if (!name || !message) {
        showResponse('Harap isi nama dan pesan Anda.', 'error');
        return;
    }

    const text = `📩 Pesan Baru dari VibeTube:\n\n👤 Nama: ${name}\n📝 Pesan: ${message}`;
    const url = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`;
    
    showResponse('Mengirim pesan...', 'loading');
    ...
    .then(data => {
        if (data.ok) {
            showResponse('Pesan terkirim! Terima Kasih Atas Masukan Atau Dukungan Yang Di Berikan ❤️', 'success');
            document.getElementById('contactName').value = '';
            document.getElementById('contactMessage').value = '';
        } else {
            showResponse('Gagal mengirim pesan. Silakan coba lagi.', 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showResponse('Terjadi kesalahan. Silakan coba lagi nanti.', 'error');
    });
}
...
function showResponse(message, type) {
    ...
    responseDiv.className = `contact-response ${type}`;
